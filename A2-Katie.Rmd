...
csb.df = read_csv(paste0('C:/Users/student/Desktop/MA710/Tuesday version/Assignment 1/CollegeScorecard_Raw_Data/', file_name), na = c("", "NA", "NULL", "PrivacySuppressed"))
...
```
Based on the DBSCAN result, there are 6 clusters in the our dataset. We subset 6 clusters from original dataset to get bettter understand of each cluster. 
```{r}
...
summary(cluster0)
```
Based on the statistical descriptions of 6 clusters, for variable control, cluster 1 is different from other 4 cluster. 
...
Now we will visually present the clusters. We will use the most common variables which describe the clusters based on teh association rules - CONTROL, NUMBRANCH, FAMINC, FEMALE, PREDDEG. Since CURROPER has the same value of 1 for both clusters, we won't use this variable for visualization purposes. 
